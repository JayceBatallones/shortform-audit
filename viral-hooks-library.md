# 1,000 Viral Hooks Library

A categorized library of plug-and-play hook templates for short-form video content. Replace the `(insert X)` placeholders with your niche-specific content.

**Source:** PBL (PersonalBrandLaunch)
**Total hooks:** 1037 (deduplicated)

---

## Educational (494 hooks)

*Teaching, tips, tutorials, how-tos, and knowledge drops. Best for top-of-funnel reach.*
...
- If you're a (insert target audience) and you want (insert
- dream result) by (insert avenue) then listen to this video.
- h=MWwyeWRoYXlkeXd5cA%3D%3D
- If you are (insert age group or range) do not do (insert action).
- As an (insert trait) responsible (insert age) year old witha goal to (insert goal) here are 3 things I will never regret doing.
...
- If you're trying to (insert dream results) then here is theone (insert thing) you should do.
...
- I crammed the hardest (insert noun) and (insert dreamresult).
...
- If your mom didn’t teach you how to make (insert
...
- I just made a website called (insert the longest but
...
- Most people can only do (insert action) when trying to (insert action) but as an (insert title) you should be ableto (insert action).
...
- strategy) to (insert dream result)
- Did you know that if you (insert action), (insert action), (insert action), etc.
...
- Here are # of things you have been putting off around
...
- I am not a DIY guy/girl but I am going to show you howI spent (insert $) to transform my (insert noun).
...
- Want to find out what’s hiding inside one of these (insert noun) when they are not properly
- maintenanced?
...
- This (insert noun) has something in it called (insert
- name). (insert verb) enough of these and they will kill you.
...
- 0 calorie (insert noun) I am not joking these are literally0 calories.
...
- Person #1: (insert action) I want to get a (insert dream result) before (insert time).
- So you want to impress you (insert person) when you (insert action) this (insert upcoming time).
- Person #1: What aren't my (insert noun) (insert adjective)?
...
- If you have (insert pain point) but not much time to (insert action) and you want to (insert result) then save these (insert noun) and (insert noun)
- steps/routines/tips.
...
- I have a (insert metric) (insert noun) because this is one of my
...
- occupation/skill/hobby) to realize I have been (insert action) wrong this entire time.
...
- Every (insert store type) has a million (insert noun) withdifferent (insert noun). But some of them are more special than others.
...
*Side-by-side, A vs B, this-or-that formats. Great for visual content and engagement.*
...
- restaurant/store/place) for (insert metric), and this is the same (insert noun) from (insert
...
*Challenging common beliefs, debunking misconceptions. High share potential.*
...
- # things I would never (insert action) in my (insert age range) if I wanted to (insert dream result) by (insert agerange).
...
- Everyone on the internet is going to tell you making (insert noun) is impossible. But I am going to show youhow to make them from home.
...
## Storytelling (375 hooks)

*Personal narratives, journeys, transformations, experiences. Best for middle-of-funnel connection.*
...
- X years ago I was (insert action) because I (insert pain
- point).
...
- After X years of (insert action/or habit) I
- stopped/started (insert action/or habit)
...
- The worst part about being a (insert title) is I literally donot…
...
- X days/months/years ago me and my (insert person)
...
- I am (insert name) (insert age) you might know me from my (insert accomplishment), (insert
...
- (insert action) everyday has literally changed my life. In
...
- I am on a journey to find the best (insert noun) for (insert target audience) and today we went to (insert
- location).
...
- My entire life I grew up (insert adjective) and (insert adjective) it was just my (insert person) raising the # of
- us.
...
- Is it possible to stop (insert pain point) and (insert dream result) in (insert time frame) even if you’re in
- (insert noun) and all of your (insert person) think getting (insert verb) is (insert trait).
...
- I have been a (insert title) for # years of my life I started when I was # years old working at (insert
...
- You know when I started making (insert noun) I shouldhave known this day would come.
...
- (insert verb) (insert noun), (insert action), and don’t
- (insert result). Welcome to (insert event).
- BSl/?igsh=MXZweHoyNGU0Z201cg==
...
- So I am (insert age) and I work nights at (insert store)
- that’s the dream life right?
- i/?igsh=Z244Y3Q0NTVvM21l
...
- A little over (insert time) ago I posted a video about thebenefits of (insert action).
...
- I bought this (insert noun) for (insert $), and I am goingto make it worth over (insert $) without changing the product in any way.
...
*Establishing expertise, credentials, and insider knowledge. Best for bottom-of-funnel trust.*
...
- If I were to create a collab campaign for (insert big
- brands) here's how I would do it.
...
## Day In The Life (17 hooks)

*Behind-the-scenes, routine, lifestyle content. Builds parasocial connection.*
...
*Unconventional, unexpected, or pattern-breaking hooks that defy categorization.*
